...
const FLOWS = {
  start: {
    botMsg:
      "Namaste 🙏 I'm the Saikripa Assistant — here to help you with fabric inquiries.\n\nWhat can I help you with today?",
    options: [
  ...
  fabrics: {
    botMsg:
      "We have 6 premium collections, all manufactured in Bhilwara 🏭\n\nOur bestsellers:\n• Superior Collection — 380 GSM, 65/35 PV\n• Gold Club — 340 GSM, premium suiting\n• Aura Plus — soft, durable, vibrant shades\n\nWould you like to see the full catalog or get details on a specific fabric?",
    options: [
      {
        label: "💬 Get Full Catalog on WhatsApp",
        action: "whatsapp",
        message: "Hi, I'd like to see your full fabric catalog. Please share details.",
      },
      {
        label: "💬 Ask About Specific Fabric",
        action: "whatsapp",
        message: "Hi, I'd like details on: ___ (please specify fabric name)",
      },
    ...
    options: [
      {
        ...
        action: "whatsapp",
        message: "Hi, I'd like a price quote.\nFabric: ___\nQuantity: ___ meters\nCity: ___",
      },
      { label: "🔙 Back to Menu", next: "start" },
    ],
  },
    ...
    options: [
      {
        ...
        action: "link",
        url: "https://maps.app.goo.gl/pps1Wfq2oJKSfkdk6",
      },
      {
        label: "💬 WhatsApp Before Visiting",
        action: "whatsapp",
        message:
          "Hi, I'd like to visit your shop in Bhilwara. When would be a good time?\nI'm interested in: ___",
      },
      { label: "🔙 Back to Menu", next: "start" },
    ],
  },
  other: {
    botMsg:
      "Sure! For any other questions, the fastest way is to message us directly on WhatsApp. Our team responds within 4 hours on business days (Mon–Sat 10–7).\n\nYou can also email saikripatextiles58@gmail.com",
    options: [
      {
        label: "💬 Chat on WhatsApp",
        action: "whatsapp",
        message: "Hi, I have a question about Saikripa Textiles fabrics.",
      },
      { label: "🔙 Back to Menu", next: "start" },
    ],
  },
  ...
  const handleOption = (option) => {
    ...
    setTimeout(() => {
      ...
      if (option.action === "whatsapp") {
        ...
        setMessages((m) => [
          ...m,
          { from: "bot", text: "Opening Google Maps for directions 📍\n\nAnything else I can help with?" },
        ]);
        setShowOptions(true);
      } else if (option.next) {
        ...
        <div className="fixed bottom-5 right-5 z-50 w-[calc(100vw-2.5rem)] sm:w-[380px] max-w-[400px] h-[calc(100vh-5rem)] sm:h-[580px] sm:max-h-[80vh] bg-[#0a1124] border border-[#d4af37]/30 rounded-[24px] shadow-[0_20px_60px_rgba(0,0,0,0.6),0_0_40px_rgba(212,175,55,0.15)] flex flex-col overflow-hidden animate-slideUp">
          ...
          <div className="bg-gradient-to-r from-[#f4d77a] via-[#d4af37] to-[#a8842c] px-5 py-4 flex items-center justify-between flex-shrink-0">
            ...
            <button
              ...
              className="text-[#020817] hover:bg-[#020817]/10 w-8 h-8 rounded-full flex items-center justify-center transition"
              ...
              <div key={i} className={`flex ${msg.from === "user" ? "justify-end" : "justify-start"} animate-fadeInUp`}>
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm whitespace-pre-line leading-relaxed ${
                    msg.from === "user"
                      ? "bg-gradient-to-br from-[#d4af37] to-[#a8842c] text-[#020817] font-medium rounded-br-sm"
                      : "bg-[#0a1124] border border-[#1a2233] text-[#e8edf5] rounded-bl-sm"
                  }`}
                >
                  {msg.text}
                </div>
            ...
            <div className="border-t border-[#1a2233] bg-[#0a1124] p-3 space-y-2 max-h-[240px] overflow-y-auto flex-shrink-0">
              {currentOptions.map((opt, i) => (
                <button
                  ...
                  className="w-full text-left bg-[#020817] border border-[#1a2233] hover:border-[#d4af37]/50 hover:bg-[#0e1730] text-[#e8edf5] hover:text-[#d4af37] text-xs font-bold uppercase tracking-wider px-4 py-3 rounded-xl transition flex items-center justify-between gap-2"
                >
                  <span>{opt.label}</span>
                  <span className="text-[#d4af37] flex-shrink-0 transition-transform group-hover:translate-x-1">→</span>
                </button>
              ))}
            </div>
          )}
          ...
          {/* Footer */}
          <div className="border-t border-[#1a2233] bg-[#020817] px-4 py-2.5 text-center flex-shrink-0">
            <p className="text-[9px] text-[#7a8499] uppercase tracking-widest font-bold">
              Mon–Sat 10AM–7PM · WhatsApp +91 89498 81253
            </p>
          </div>
        </div>
      )}
